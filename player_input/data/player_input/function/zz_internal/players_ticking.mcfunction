execute \
    if score players player_input.listeners matches 1.. run \
    schedule function player_input:zz_internal/players_ticking 1t replace

execute if score player_tracking player_input.listeners matches 1.. run function player_input:zz_internal/players_tracking
...
tag @a[predicate=player_input:forward] add player_input.forward
tag @a[predicate=player_input:backward] add player_input.backward
tag @a[predicate=player_input:left] add player_input.left
tag @a[predicate=player_input:right] add player_input.right
tag @a[predicate=player_input:jump] add player_input.jump
tag @a[predicate=player_input:sneak] add player_input.sneak
tag @a[predicate=player_input:sprint] add player_input.sprint
...
execute as @a run scoreboard players operation @s player_input.previous_tick = @s player_input.current_tick

scoreboard players set @a player_input.current_tick 0
...
scoreboard players set @a player_input.changed 0
execute as @a if score @s player_input.current_tick = @s player_input.previous_tick run \
    scoreboard players set @s player_input.changed 1

execute if score player.key_update player_input.listeners matches 1.. \
    as @a[scores={player_input.changed=1}] at @s run \
